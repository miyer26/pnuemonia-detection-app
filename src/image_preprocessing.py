...
class Transformation:
    ...
    def _get_base_transforms(self):
        ...
        transform_seq = create_transform(**resolve_data_config(model.pretrained_cfg, model=model))
        return transform_seq
    
    @property
    def augmented_transforms(self):
        if self._augmentation_transforms_list is not None:
            return transforms.Compose([self.base_transform] + self._augmentation_transforms_list)
        else:
